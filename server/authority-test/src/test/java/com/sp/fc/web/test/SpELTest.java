...
public class SpELTest {
    ...
    void test_4() {
        ...
        assertFalse(parser.parseExpression("#horse.over(170)")
                .getValue(ctx, boolean.class));

        assertTrue(parser.parseExpression("over(170)")
                .getValue(ctx, boolean.class));

        assertTrue(parser.parseExpression("@person.over(170)")
                .getValue(ctx, boolean.class));
        assertFalse(parser.parseExpression("@nancy.over(170)")
                .getValue(ctx, boolean.class));
    }
    ...
    void test_3() {
        assertTrue(parser.parseExpression("over(170)")
                .getValue(person, boolean.class));
        assertFalse(parser.parseExpression("over(170)")
                .getValue(nancy, boolean.class));
    }
